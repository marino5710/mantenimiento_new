            ...
            render: (data, type, row) => {
                ...
                <div class='text-center'>
                    <button class="btn btn-warning btn-sm rounded-circle editar" data-bs-toggle='modal' data-bs-target='#modalNuevo' title='Modificar'
                        data-codigo='${data}' data-nombre='${row['categoria_nombre']}' data-descripcion='${row['categoria_descripcion'] || ''}'>
                        <i class='fas fa-file-pen fa-xs'></i>
                    </button>`;
                if (row.categoria_situacion == 1) {
...
const buscarApi = async () => {
    try {
        const url = `/API/categorias/buscar`;
...
const guardarApi = async e => {
    ...
    if (!validarFormulario(formPrincipal, ['categoria_id'])) {
    ...
    try {
        const url = `/API/categorias/guardar`;
        const body = new FormData(formPrincipal);
        body.delete('categoria_id');
...
const modificarApi = async e => {
    ...
    try {
        const url = `/API/categorias/modificar`;
        const body = new FormData(formPrincipal);
...
const eliminarApi = async (e) => {
    ...
    const confirm = await confirmacion('¿Desea desactivar esta categoría?', 'warning', 'Sí, desactivar');
    if (confirm) {
        const body = new FormData();
        body.append('categoria_id', id);
        body.append('categoria_situacion', 0);

        const respuesta = await fetch(`/API/categorias/eliminar`, {
            method: 'POST',
            body,
            headers: new Headers({ 'X-Requested-With': 'fetch' })
        });
...
const activarApi = async (e) => {
    const id = e.currentTarget.dataset.codigo;
    const confirm = await confirmacion('¿Desea activar esta categoría?', 'warning', 'Sí, activar');
    if (confirm) {
        const body = new FormData();
        body.append('categoria_id', id);
        body.append('categoria_situacion', 1);
...
const asignarValores = (e) => {
    ...
    formPrincipal.categoria_id.value = data.codigo;
    formPrincipal.categoria_nombre.value = data.nombre;
    formPrincipal.categoria_descripcion.value = data.descripcion;
...
const resetearModal = () => {
    modalTitleId.textContent = 'Ingresar Categoría';
